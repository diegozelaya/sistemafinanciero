...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function cargarTabla(facturas) {
        ...
        facturas.forEach(f => {
            ...
            const botonEditar = f.activo === 1
                ? `<button class="btn btn-sm btn-warning editar-btn" data-id="${f.idventa}">✏ Editar</button>`
    ...
    document.addEventListener('click', async (e) => {
        if (e.target.classList.contains('editar-btn')) {
            ...
            // Opcional: obtener datos de la factura vía AJAX
            const res = await fetch(`/detalle/${id_venta}`);
            const factura = await res.json();

            // Asignar datos al modal
            document.getElementById('editFecha').value = factura.fecha; // asegurate que sea formato YYYY-MM-DD
            ...
            // Mostrar el modal
            const modal = new bootstrap.Modal(document.getElementById('modal_edit'));
            modal.show();
        }
    });
