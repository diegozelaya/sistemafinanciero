...
$(document).ready(function () {
  ...
  $('#productForm').on('submit', function (e) {
    ...
    $('#contratoSi').off('click').on('click', function () {
      enviarMatricula(1);  // contrato = 1
    });

    $('#contratoNo').off('click').on('click', function () {
      enviarMatricula(0);  // contrato = 0
    });
  });

  // Función que arma el formData y hace el AJAX
  function enviarMatricula(contrato) {
    ...
    var formData = {
      ...
      detalles: detalles,
      contrato: contrato   // 🔹 valor según respuesta del modal
    };
